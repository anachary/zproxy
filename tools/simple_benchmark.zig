...
pub fn runBenchmark(allocator: std.mem.Allocator, config: BenchmarkConfig) !BenchmarkStats {
    ...
    // Initialize with the concurrency value
    ...
    // Create worker threads - use more threads for higher concurrency
    const thread_count: usize = if (config.concurrency > 100000)
        64 // Use 64 threads for extreme concurrency
    else if (config.concurrency > 10000)
        32 // Use 32 threads for high concurrency
    else
        @min(config.concurrency, 16); // Use up to 16 threads for normal concurrency
...
fn workerThread(context: *WorkerContext) !void {
    ...
    var http_request_buffer: [256]u8 = undefined;
    var http_request: []const u8 = "";

    if (config.send_http_request) {
        http_request = try std.fmt.bufPrint(
            &http_request_buffer,
            "GET {s} HTTP/1.1\r\nHost: {s}:{d}\r\nConnection: {s}\r\n\r\n",
            .{
                config.http_path,
...
pub fn main() !void {
    // Initialize logger
    const logger = std.log.scoped(.benchmark);
    logger.info("Starting connection benchmark...", .{});

    // Default configuration
    var config = BenchmarkConfig{
        .host = "127.0.0.1",
        .port = 8080,
        .concurrency = 10000, // Increased from 1000 to 10000
        .duration_seconds = 30, // Increased from 10 to 30
        .keep_alive = false,
    ...
    while (args_it.next()) |arg| {
        if (std.mem.eql(u8, arg, "--host")) {
            ...
            if (args_it.next()) |value| {
                config.concurrency = try std.fmt.parseInt(usize, value, 10);
            }
        } else if (std.mem.eql(u8, arg, "--duration")) {
            if (args_it.next()) |value| {
                config.duration_seconds = try std.fmt.parseInt(u64, value, 10);
            }
    ...
    // Create a page allocator for large allocations
    const page_allocator = std.heap.page_allocator;

    // Run the benchmark
    var stats = try runBenchmark(page_allocator, config);
...
fn printUsage() void {
    ...
    stdout.print(
        ...
        \\  --duration SECONDS      Benchmark duration in seconds (default: 10)
